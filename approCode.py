...
# srclat, srclng, destlat, destlng = 90.363833, 23.834145, 90.37836, 23.75693
srclat, srclng, destlat, destlng = 90.40023, 23.87596, 90.367982, 23.835966
# srclat, srclng, destlat, destlng = 90.36295, 23.80874, 90.40737, 23.73176
# srclat, srclng, destlat, destlng = 90.439412, 23.737918, 90.439743, 23.733974
...
for i in range(0, nodes_so_far):
    print(needed_time[i], i)
...
while True:
    ...
    t_cost, t_time = visited_nodemap[current.id]
    #
    if (current.cost_so_far > t_cost):
        continue
    ...
    for next in edgemap[current.id]:
        ...
        next_time += waiting_time(next[3], current.time_so_far)
        next_lat, next_lng = reverse_nodemap[next_id]
        # print(visited_nodemap[next_id][0], ' test print ', next_cost)
        if next_cost >= visited_nodemap[next_id][0]:
            continue

        if next_time + needed_time[next_id] <= end_time_min:
            print(' HERE >>>>>>>>>>>> ', needed_time[next_id])
            ...
            if (next_cost < t_cost) and (t_time + needed_time[next_id] <= end_time_min):
...
#         if j%2==0:
#             out_kml.write(str(tokens[j]) + "," + str(tokens[j+1]) + ",0\n")
#
